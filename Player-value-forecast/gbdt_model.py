...
# 用非守门员数据训练随机森林
model = GradientBoostingRegressor(loss='ls', learning_rate=0.03, n_estimators=500, subsample=0.8
                                  , min_samples_split=50, min_samples_leaf=10, max_depth=4
                                  , init=None, random_state=None, max_features=None
                                  , alpha=0.9, verbose=0, max_leaf_nodes=None
                                  , warm_start=False
                                  )
model.fit(train[train['is_gk'] == False][cols], train[train['is_gk'] == False]['y'])

preds = model.predict(test[test['is_gk'] == False][cols])
...
model.fit(train[train['is_gk'] == True][cols], train[train['is_gk'] == True]['y'])

preds = model.predict(test[test['is_gk'] == True][cols])
